...
const resources = [
  ...
  },
  {
    ...
    name: 'Hyros Tags',
    description:
      'All available tags in the Hyros account. Product tags start with $, source tags with @, action tags with !',
    mimeType: 'application/json',
  },
  {
    uri: 'hyros://stages',
...
const prompts = [
  {
    ...
    arguments: [
      { name: 'date', description: 'Date in YYYY-MM-DD format (defaults to today)', required: false },
      { name: 'timezone', description: 'Timezone offset (e.g., -05:00 for EST)', required: false },
    ],
  },
  {
    ...
    arguments: [
      { name: 'fromDate', description: 'Start date in ISO 8601 format', required: true },
      { name: 'toDate', description: 'End date in ISO 8601 format', required: true },
      {
        name: 'platform',
  ...
  },
  {
    name: 'lead_lookup',
    description:
      'Investigate everything about a specific lead: their profile, purchase history, call history, and ad attribution journey',
    arguments: [
      { name: 'email', description: 'Lead email address', required: false },
      { name: 'leadId', description: 'Lead ID', required: false },
    ],
  },
];
...
export class HyrosMCP extends McpAgent<Env, unknown, HyrosProps> {
  ...
  async init(): Promise<void> {
    ...
    if (!apiKey) {
      throw new Error('Missing Hyros API key in authorization context.');
    }
    const baseUrl = this.env.HYROS_BASE_URL ?? 'https://api.hyros.com/v1';
    ...
    server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
      ...
      try {
        ...
        if (uri === 'hyros://account') {
          ...
          return {
            contents: [
              { uri, mimeType: 'application/json', text: JSON.stringify({ error: `Unknown resource: ${uri}` }) },
            ],
          };
        }
...
Be thorough and organized.`,
        ...
        default:
          return {
            ...
            messages: [{ role: 'user', content: { type: 'text', text: `Prompt "${name}" not found.` } }],
          };
      }
    });
  }
